...
@Service
public class ReviewerStrategy implements RoleStrategy {
    @Autowired
    private PurchaseOrderRepository repository;

    @Override
    public List<PurchaseOrderResponseDTO> listPurchaseOrders() {
        List<String> statuses = Arrays.asList("created", "reworked");
        List<PurchaseOrder> purchaseOrders = repository.findByStatusIn(statuses);
